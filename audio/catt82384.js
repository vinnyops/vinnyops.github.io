...
(function() {
	...
	addReplacement('VERSION$1," | ",', `"Cattsforlife's Miniblox Hacks Beta 10 Final"," | ",`);
	addReplacement('if(!nt.canConnect){', 'nt.errorMessage = nt.errorMessage == "Could not join server. You are connected to a VPN or proxy. Please disconnect from it and refresh the page." ? "Your using a VPN or You have been IP banned (sad sad)" : nt.errorMessage;');
	...
	addReplacement('(this.drawSelectedItemStack(),this.drawHintBox())', `
		if (ctx$3 && enabledModules["TextGUI"]) {
			...
			// Use solid color instead of dynamic HSL color
			const solidColor = "#FF0000"; // Example: solid red color
			...
			// Draw module names with solid color
...
/		}
	...
	addReplacement('async downloadCape(_){', `
		if (_ == "GrandDad") {
			const $ = capes[_];
			return new Promise((et, tt) => {
				textureManager.loader.load("https://raw.githubusercontent.com/vinnyops/vinnyops.github.io/refs/heads/main/textures/entity/capes/bao.png", rt => {
					const nt = {
						atlas: rt,
						id: _,
						name: $.name,
